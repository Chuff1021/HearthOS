...
import { buildVentingAnswerFromRecord, classifyVentingQuestionType, extractVentRuleRecords, pickBestVentRule, VentRuleRecord } from "../src/swarm/ventingEngine";

type FailureBucket = "scorer mismatch" | "retrieval gap";
...
function legacyClassifyVentingQuestionType(question: string) {
...
function legacyExtractVentRuleRecords(chunks: RetrievedChunk[]): VentRuleRecord[] {
  return chunks.flatMap((c) => {
    if (c.source_type !== 'manual') return [];
    const text = (c.chunk_text || '').replace(/\s+/g, ' ');
    const lc = text.toLowerCase();
    if (!/vent|termination|horizontal|vertical|elbow|pipe/.test(lc)) return [];

    const has = (re: RegExp) => re.test(text);
    const hasAny =
      has(/\b(4\s*[x×]\s*6\s*5?\b|5\s*[x×]\s*8\b|3\s*[x×]\s*5\b)/i) ||
    ...
    return extractVentRuleRecords([c]);
  });
}

function evaluate(cases: Case[], mode: "legacy" | "current") {
  const rows: any[] = [];
  let passed = 0;

  for (const c of cases) {
    const chunk = mkChunk(c);
    const recs = mode === "legacy" ? legacyExtractVentRuleRecords([chunk]) : extractVentRuleRecords([chunk]);
    const qtype = mode === "legacy" ? legacyClassifyVentingQuestionType(c.question) : classifyVentingQuestionType(c.question);
    ...
    const answer = best ? buildVentingAnswerFromRecord(best, c.question) : null;
    const bucket: FailureBucket | null = pass ? null : (!answered ? "retrieval gap" : "scorer mismatch");
  ...
  for (const f of failures) {
    if (!f.exact_failure_bucket) continue;
...
function main() {
  ...
  const outPath = path.join(__dirname, "venting_32_hardening_report.json");
