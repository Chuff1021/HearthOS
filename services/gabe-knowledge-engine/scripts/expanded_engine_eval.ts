...
import { buildVentingAnswerFromRecord, classifyVentingQuestionType, extractVentRuleRecords, pickBestVentRule } from "../src/swarm/ventingEngine";
import { buildWiringAnswerFromRecord, classifyWiringQuestionType, extractWiringRecords, pickBestWiringRecord } from "../src/swarm/wiringEngine";
...
type Cause = "extraction gap" | "retrieval gap" | "validator/gating issue" | "source-evidence limitation" | "scorer mismatch";

type EvalCase = {
  id: string;
  engine: "venting" | "wiring" | "parts" | "compliance";
  question: string;
  expectedQtype: string;
  shouldAnswer: boolean;
  chunk: string;
  score?: number;
};

function mkChunk(c: EvalCase): RetrievedChunk {
  return {
...
function baselineVentingQType(question: string) {
  const q = question.toLowerCase();
  if (q.includes("horizontal")) return "max_horizontal_run";
  if (q.includes("vertical")) return "max_vertical_run";
  if (q.includes("elbow")) return "elbow_offset_effect";
  if (q.includes("termination") || q.includes("window") || q.includes("door")) return "termination_restriction";
  return "general_venting";
}

function baselineWiringQType(question: string) {
  const q = question.toLowerCase();
  if (q.includes("switch")) return "switch_module_relationship";
  if (q.includes("valve")) return "valve_module_relationship";
  if (q.includes("transformer")) return "transformer_relationship";
  return "connection_path";
}

function baselinePartsQType(question: string) {
  const q = question.toLowerCase();
  if (q.includes("diagram") || q.includes("callout")) return "diagram_callout";
  if (q.includes("revision") || q.includes("family")) return "revision_disambiguation";
  if (q.includes("alias") || q.includes("aka")) return "alias_lookup";
  return "part_lookup";
}

function baselineComplianceQType(question: string) {
  const q = question.toLowerCase();
  if (q.includes("listing") || q.includes("csa") || q.includes("ul")) return "listing";
  if (q.includes("approval") || q.includes("approved")) return "approval";
  if (q.includes("inspection")) return "inspection";
  if (q.includes("permit")) return "permit";
  if (q.includes("code")) return "code_vs_manufacturer";
  return "clearance_compliance";
}

function baselineShouldAnswer(c: EvalCase) {
  if (c.engine === "compliance") return /listed|approved|inspection|permit|code|clearance/i.test(c.chunk);
  if (c.engine === "parts") return /part|callout|sku|p\/n|revision|alias/i.test(c.chunk);
  if (c.engine === "wiring") return /wiring|switch|module|receiver|transformer|valve/i.test(c.chunk);
  return /vent|termination|horizontal|vertical|elbow|pipe/i.test(c.chunk);
}

function classifyCause(engine: string, shouldAnswer: boolean, answered: boolean, qtypeOk: boolean, notes: string[]): Cause {
  const blob = notes.join("|").toLowerCase();
  if (shouldAnswer && !answered) return blob.includes("missing") ? "source-evidence limitation" : "retrieval gap";
  if (!shouldAnswer && answered) return "validator/gating issue";
  if (!qtypeOk) return "scorer mismatch";
  if (blob.includes("missing_fields")) return "extraction gap";
  return engine === "compliance" ? "validator/gating issue" : "extraction gap";
}

function top3Failures(rows: { pass: boolean; cause?: Cause }[]) {
  const counts = new Map<Cause, number>();
  for (const r of rows) {
    if (!r.pass && r.cause) counts.set(r.cause, (counts.get(r.cause) || 0) + 1);
  }
  return [...counts.entries()].sort((a, b) => b[1] - a[1]).slice(0, 3).map(([cause, n]) => ({ cause, count: n }));
}

function evaluateEngine(engine: EvalCase["engine"], cases: EvalCase[]) {
  const rows: any[] = [];
  let beforePassed = 0;
  let afterPassed = 0;
  ...
  for (const c of cases) {
    ...
    if (engine === "venting") {
      ...
      aq = classifyVentingQuestionType(c.question);
      const recs = extractVentRuleRecords([chunk]);
      const best = pickBestVentRule(c.question, recs);
      answered = Boolean(best);
      notes = best ? (buildVentingAnswerFromRecord(best, c.question) as any).validator_notes || [] : ["source_not_found"];
    } else if (engine === "wiring") {
      bq = baselineWiringQType(c.question);
      aq = classifyWiringQuestionType(c.question);
      const recs = extractWiringRecords([chunk]);
      const best = pickBestWiringRecord(c.question, recs);
      answered = Boolean(best);
      notes = best ? (buildWiringAnswerFromRecord(best, c.question, recs) as any).validator_notes || [] : ["source_not_found"];
    } else if (engine === "parts") {
      bq = baselinePartsQType(c.question);
      aq = classifyPartsQuestionType(c.question);
      const recs = extractPartsRecords([chunk]);
      const best = pickBestPartsRecord(c.question, recs);
      answered = Boolean(best);
      notes = best ? (buildPartsAnswerFromRecord(best, c.question, recs) as any).validator_notes || [] : ["source_not_found"];
    } else {
      bq = baselineComplianceQType(c.question);
      aq = classifyComplianceQuestionType(c.question);
      const recs = extractComplianceRecords([chunk], c.question);
      const best = pickBestComplianceRecord(c.question, recs);
      answered = Boolean(best);
      const ans = best ? buildComplianceAnswerFromRecord(best, c.question) : buildComplianceRefusal(c.question);
      notes = (ans as any).validator_notes || [];
    }
    ...
    if (beforePass) beforePassed += 1;
    if (afterPass) afterPassed += 1;
...
function buildCases(): Record<EvalCase["engine"], EvalCase[]> {
  const venting: EvalCase[] = [];
  const ventPrompts = [
...
function main() {
  ...
  const outPath = path.join(__dirname, "expanded_engine_eval_report.json");
  fs.writeFileSync(outPath, JSON.stringify(report, null, 2));

  console.log("=== EXPANDED ENGINE EVAL (32 cases each) ===");
  for (const key of ["venting", "wiring", "parts", "compliance"] as const) {
    const r: any = (report as any)[key];
