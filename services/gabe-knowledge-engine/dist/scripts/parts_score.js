...
function baselineClassify(question) {
    ...
    if (/diagram|callout|exploded/.test(q))
        return "diagram_callout";
    if (/revision|rev\b|variant|family|series/.test(q))
        return "revision_disambiguation";
    if (/alias|also called|aka|same as/.test(q))
        return "alias_lookup";
    return "part_lookup";
}
function baselineCanonicalize(text) {
...
function main() {
    ...
    for (const c of cases) {
        ...
        const aq = (0, partsEngine_1.classifyPartsQuestionType)(c.question);
        if (bq === c.expectedQtype)
            beforeQ += 1;
        if (aq === c.expectedQtype)
            afterQ += 1;
        const bp = baselineCanonicalize(c.chunk);
        const ap = (0, partsEngine_1.canonicalizePartNumbers)(c.chunk);
    ...
    const outPath = node_path_1.default.join(__dirname, "parts_score_report.json");
