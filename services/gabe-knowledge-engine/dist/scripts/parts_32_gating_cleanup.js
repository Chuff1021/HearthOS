...
const validatorAgent_1 = require("../src/swarm/validatorAgent");
function mkCases() {
    const seeds = [
...
function legacyExtractPartsRecords(chunks) {
    const out = [];
    for (const c of chunks) {
        ...
        if (partNums.length + callouts.length + aliases.length === 0)
            continue;
        out.push({
...
function evaluate(mode) {
    ...
    for (const c of cases) {
        ...
        const qtype = (0, partsEngine_1.classifyPartsQuestionType)(c.question);
        const records = mode === "legacy" ? legacyExtractPartsRecords([chunk]) : (0, partsEngine_1.extractPartsRecords)([chunk]);
        ...
        if (best) {
            answer = (0, partsEngine_1.buildPartsAnswerFromRecord)(best, c.question, records);
            const verdict = (0, validatorAgent_1.validateOrReject)(answer, [chunk]);
            if (verdict.ok) {
...
function main() {
    ...
    const outPath = node_path_1.default.join(__dirname, "parts_32_gating_cleanup_report.json");
    node_fs_1.default.writeFileSync(outPath, JSON.stringify(out, null, 2));
