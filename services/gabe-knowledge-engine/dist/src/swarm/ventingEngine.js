...
exports.extractVentRuleRecords = extractVentRuleRecords;
exports.pickBestVentRule = pickBestVentRule;
exports.classifyVentingQuestionType = classifyVentingQuestionType;
exports.buildVentingAnswerFromRecord = buildVentingAnswerFromRecord;
function extractVentRuleRecords(chunks) {
    const out = [];
...
function pickBestVentRule(question, records) {
    if (records.length === 0)
        return null;
    const q = question.toLowerCase();
    const qNums = new Set((question.match(/\b\d{2,3}\b/g) || []).map((n) => String(n)));
    const hasAny = (...arr) => arr.some((a) => q.includes(a));
    const byModel = records
        ...
        if (m && q.includes(m))
            bonus += 18;
        if (qNums.size > 0 && [...qNums].some((n) => m.includes(n)))
            bonus += 14;
        if (hasAny("vent table", "vent chart") && (r.required_conditions || "").toLowerCase().includes("vent"))
            bonus += 10;
        if (hasAny("termination", "window", "door", "opening") && (r.termination_constraints || r.window_door_clearance_notes))
            bonus += 12;
        if (hasAny("elbow", "offset", "90-degree", "90 degree") && (r.elbow_offset_constraints || r.equivalent_run_penalty))
            bonus += 12;
        if (hasAny("vertical", "rise", "limit") && (r.min_rise || r.max_vertical))
            bonus += 10;
        if (hasAny("horizontal", "hori") && r.max_horizontal)
            bonus += 10;
        if (hasAny("pipe", "direct vent", "dv") && r.vent_system_pipe_type)
            bonus += 10;
        if (hasAny("family", "approved") && r.approved_vent_family)
            bonus += 10;
...
function classifyVentingQuestionType(question) {
    const q = question.toLowerCase();
    const has = (x) => q.includes(x);
    const hasAny = (...arr) => arr.some((a) => q.includes(a));
    if (hasAny('minimum', 'min') && has('rise'))
        return 'min_rise_before_horizontal';
    if (hasAny('maximum', 'max') && hasAny('horizontal', 'hori'))
        return 'max_horizontal_run';
    if (hasAny('maximum', 'max', 'limit') && hasAny('vertical', 'vert'))
        return 'max_vertical_run';
    if (hasAny('elbow', 'offset', '90-degree', '90 degree'))
        return 'elbow_offset_effect';
    if (hasAny('termination', 'window', 'door', 'opening'))
        return 'termination_restriction';
    if (hasAny('pipe', 'vent family', 'approved', 'dv', 'direct vent'))
        return 'pipe_or_family';
    if (hasAny('vent table', 'vent chart', 'venting applies'))
        return 'general_venting';
    return 'general_venting';
}
function buildVentingAnswerFromRecord(record, question) {
    const type = classifyVentingQuestionType(question);
    const missing = [];
    let answer = '';
    if (type === 'min_rise_before_horizontal') {
        if (!record.min_rise)
            missing.push('min_rise');
        ...
        if (!record.max_horizontal)
            missing.push('max_horizontal');
        ...
        if (!record.max_vertical)
            missing.push('max_vertical');
        ...
        if (!record.elbow_offset_constraints && !record.equivalent_run_penalty)
            missing.push('elbow/equivalent_run_penalty');
        ...
        if (!record.termination_constraints && !record.window_door_clearance_notes)
            missing.push('termination/window_door_clearance_notes');
        ...
        if (record.termination_exceptions)
            answer += ` Exceptions: ${record.termination_exceptions}.`;
    }
    else if (type === 'pipe_or_family') {
        if (!record.vent_system_pipe_type && !record.approved_vent_family)
            missing.push('vent_system_pipe_type/approved_vent_family');
    ...
    let certainty = record.confidence >= 85 ? 'Verified Exact' : 'Verified Partial';
    if (missing.length) {
...
function norm(v) { return v ? v.replace(/\s+/g, ' ').trim() : undefined; }
