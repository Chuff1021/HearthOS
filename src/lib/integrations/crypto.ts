...
function encryptionKey() {
  ...
  if (!raw) throw new Error("INTEGRATION_ENCRYPTION_KEY is required for tenant integrations.");

  if (/^[a-f0-9]{64}$/i.test(raw)) return Buffer.from(raw, "hex");
  const decoded = Buffer.from(raw, "base64");
  if (decoded.length === 32) return decoded;
  throw new Error("INTEGRATION_ENCRYPTION_KEY must be 32 bytes encoded as base64 or 64 hex characters.");
}
